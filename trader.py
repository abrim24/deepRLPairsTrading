...
class TraderEnv(gym.Env):
    ...
    def __init__(self,p1,p2,trainIters=100):
        #trading params
        ...
        #matplotlib
        self.f, self.axarr = plt.subplots(4, sharex=True, figsize=(7,10))
    ...
    def step(self, action):
        ...
        if self.training and rewards > 0.0: rewards *= 2.0 # making positive returns much greater when training
        ...
        if done and not self.training:
            ...
            self.axarr[3].set_xlabel("Test Data Trading Days",fontsize=15)
            #self.axarr[3].figure(figsize=(100,100))
            plt.savefig(str(self.pairs[0][0])+str(self.pairs[0][1])+".test_r.eps"+str(self.trainMax)+".png")
            plt.show()
            #plt.close()
    ...
    def reset(self):
        self.dataidx = self.lags if self.training else self.testIdx
